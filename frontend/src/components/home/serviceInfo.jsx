...
export default async function ServiceInfo(data) {
    ...
    const setServiceItems = data.setServiceItems
    const setIisServiceOpen = data.setIisServiceOpen
                                ...
                                service.packServices.map((p, i) => (
                                    {
                                        ...
                                        type: p.type.name,
                                        status: p.status.name,
                                        aes: (<a href={`https://boss.comfortel.pro/index.phtml?service_id=${p.id}&url_fav=1&mid=154&pid=404&module_mode=open&company_id=${p.companyId}&oid=1163`}>AES</a>),
                                    }
                                ))}
                            columns={[
                                {
    ...
    setServiceItems([
        <Row key={1}>
            <Col key={1} xs={{flex: '100%'}} md={{flex: '1 2'}} style={{padding: 20}}>
            ...
            <Col key={2} flex='2 1' style={{padding: 20}}>
